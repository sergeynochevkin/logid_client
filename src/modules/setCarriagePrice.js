export const setCarriagePrice = (
  prices,
  distance,
  formData,
  additional_points_quantity
) => {
  let price;
  ...
  if (formData.type.value) {
    ...
    if (!formData.load_capacity.value) {
      step = prices.find(
        (el) => el.range > distance / 1000 && el.type === formData.type.value
      );
    } else {
      step = prices.find(
        (el) =>
          el.range > distance / 1000 &&
          el.type === formData.type.value &&
          el.load_capacity === formData.load_capacity.value
      );
    }
