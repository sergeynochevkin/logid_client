...
const DriverItem = observer(({ driver }) => {
  ...
  useEffect(() => {
    if (Driver.images.find((el) => el.id === driver.user_info.id)) {
      setImages(
        Driver.images.find((el) => el.id === driver.user_info.id).urlsArray
      );
    }
  }, [Driver.images]);
  ...
  return (
    <>
      <div className={`item_container driver ${Setting.app_theme} `}>
        <div
          className={`driver_avatar_container ${Setting.app_theme}`}
          style={{
        ...
        {driver.isActivated ? (
          <>
            ...
            <CardColValue>
              {driver.user_info.complete_orders_amount}
            </CardColValue>
          </>
        ) : (
          <CardColValue>
        ...
        <div ref={ref}>
          ...
          {isComponentVisible ? (
            <div className="driver_settings_container">
              {driver.user_info.user_app_settings
                .slice()
                .sort(sortSetings)
                .filter(
                  (el) =>
