...
const PartnerModalContent = observer(
  ({ setModalActive, onePartnerInfo, onePartner }) => {
    ...
    const you_blocked = SetNativeTranslate(
      Translate.language,
      {},
      "you_blocked"
    );
    const you_have_changed_status = SetNativeTranslate(
      Translate.language,
      {},
      "you_have_changed_status"
    );
    const customer_notification = SetNativeTranslate(
      Translate.language,
      {},
      "customer_notification"
    );
    const carrier_notification = SetNativeTranslate(
      Translate.language,
      {},
      "carrier_notification"
    );
    const to_favorite_notification = SetNativeTranslate(
      Translate.language,
      {},
      "to_favorite_notification"
    );
    const to_normal_notification = SetNativeTranslate(
      Translate.language,
      {},
      "to_normal_notification"
    );
        ...
        Notification.addNotification([
          {
            id: v4(),
            type: "success",
            message: `${you_have_changed_status} ${
              user.user.role === "carrier"
                ? customer_notification
                : carrier_notification
            } ${to_normal_notification}`,
    ...
    useEffect(() => {
      if (Partner.images.find((el) => el.id === onePartnerInfo.id)) {
        setImages(
          Partner.images.find((el) => el.id === onePartnerInfo.id).urlsArray
        );
      }
    }, []);
        ...
        <div className="partner_view_container">
          ...
          <CardRow>
            ...
            <CardColName
              style={{
                backgroundColor:
                  user.user.role !== "driver" && onePartner.status === "normal"
                    ? "rgb(241,196,15,0.8)"
              ...
              <>
                {onePartner.status === "normal" ? (
                  ...
                  <>
                    ...
                    <CardButton onClick={normal}>
                      {SetNativeTranslate(
                        Translate.language,
                        {},
                        "partner_from_blocked"
                      )}
                    </CardButton>
                  </>
                ) : onePartner.status === "priority" ? (
