...
const PartnersList = observer(() => {
  ...
  const debouncedSearchTerm = useDebounce(
    FilterAndSort.partnerFilters.partners,
    500
  );
  ...
  return (
    <>
      <VerticalContainer style={{ alignItems: "center" }}>
        <FilterAndSortComponentForServer parent={"partners"} />
        ...
        <div className={"scroll_bar_container"}>
          {Partner.partnerInfos.length !== 0 ? (
                  ...
                  {Partner.partners.length > 0 &&
                    Partner.partnerInfos.map((onePartnerInfo) => (
                      <PartnerItem
                        ...
                        onePartner={Partner.partners.find(
                          (el) => el.partnerUserInfoId === onePartnerInfo.id
                        )}
                        onePartnerOtherRatingByThisUserInfo={Rating.otherRatings.find(
                          (el) => el.ratedUserInfoId === onePartnerInfo.id
                        )}
                      />
                    ))}
                </tbody>
            ...
            <NoData>
              {SetNativeTranslate(Translate.language, {}, "no_partners")}
            </NoData>
          )}
        </div>
      </VerticalContainer>
