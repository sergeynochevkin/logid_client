...
const UsersItemActionModalContent = observer(
  ({
    action,
    setAction,
    actionIcons,
    ...
    return (
      ...
      >
            ...
            {action === "mail"
              ? SetNativeTranslate(Translate.language, {
                  russian: [
                    ...
                    Management.users
                      .filter((el) => formData.members.includes(el.id))
                      .map((el) => el.email)
                      .join(", "),
                  ],
                  english: [
        ...
        <input
          style={{
            borderLeft:
              formData.subject === "" ? "rgb(254, 111, 103,0.8) solid 1px" : "",
          }}
        ...
        {action === "mail" || action === "alert" ? (
          <label className="management_checkbox_text">
            <input
              ...
              type={"checkbox"}
              checked={formData.type === "mail_alert"}
              value={checked}
              onChange={() => {
            ...
            {SetNativeTranslate(Translate.language, {
              russian: [
                "дублировать",
                action === "mail" ? "в уведомлении" : "на email",
              ],
              english: [
                "duplicate",
                action === "mail" ? "in notification" : "to email",
              ],
            })}
          </label>
        ) : (
        ...
        <div className="management_action_menu_modal_buttons_container">
          ...
          <img
            className={`management_sync_icon ${
              formData.subject === "" || !formData.messge === ""
                ? "disabled"
                : ""
            }`}
            src={actionIcons.two}
