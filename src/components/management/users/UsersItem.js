  ...
  ({
    ...
    useEffect(() => {
      if (Management.user_images.find((el) => el.id === oneUser.user_info.id)) {
        setImages(
          Management.user_images.find((el) => el.id === oneUser.user_info.id)
            .urlsArray
        );
      }
    }, []);
    ...
    return (
      <>
        <div
          style={{
            boxShadow: !formData.members.includes(oneUser.id)
              ? ""
              : `${
                  Setting.app_theme === "light"
                    ? "0px 2.5px 5px 0 rgb(129, 199, 132,0.8)"
                    : "0px 2.5px 5px 0 rgb(129, 199, 132,0.8)"
                }`,
          }}
          onClick={() => {
            ...
            if (formData.members.includes(oneUser.id)) {
              let data = [
                ...formData.members.filter((el) => el !== oneUser.id),
              ];
              setFormData({ ...formData, members: [...data] });
            }
          }}
        >
          <div className={`management_row ${Setting.app_theme}`}>
            <div
              ...
              className={`user_avatar_container ${Setting.app_theme}`}
              style={{
            ...
            >
              {!images[0] && !oneUser.user_info.email
                ? "N"
                : !images[0] && oneUser.user_info.email
                ? oneUser.user_info.email.charAt().toUpperCase()
                : ""}
            </div>
            ...
            <div className="management_item">
              {SetNativeTranslate(Translate.language, "", oneUser.role)}
            </div>
            <div className="management_item">
              {SetNativeTranslate(
                Translate.language,
                "",
                oneUser.user_info.country
              )}
            </div>
            ...
            {Object.keys(oneUser.user_info).length === 0 ? (
              <div className="management_item"> No profile! </div>
            ) : (
              <></>
            )}
          </div>
          <div className="management_more_icon_container">
            <img
              ...
              className="management_more_icon"
              src={Setting.app_theme === "light" ? more : more_dark}
            />
            {actionMenuActive && isComponentVisible ? (
              <UsersItemActionMenu
                ...
                oneUser={oneUser}
                setActionMenuActive={setActionMenuActive}
                setAction={setAction}
                action={action}
