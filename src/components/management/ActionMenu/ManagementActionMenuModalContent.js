...
const ManagementActionMenuModalContent = observer(({ action, setAction, setModalActive, formData, setFormData, initialValue }) => {
    ...
    const moderationAction = async (option, moderated) => {
        try {
            await updateField({ ...formData, option: option, moderated: moderated }).then(data => {
                Notification.addNotification([{ id: v4(), type: moderated === 'checked_accepted' ? 'success' : 'error', message: data }])
            })
    ...
    return (
        <div className={`management_action_menu_modal_container ${Setting.app_theme}`}>
            ...
            <textarea
                ...
                placeholder={'Enter a comment on the solution'} rows='5' className={`management_search ${Setting.app_theme}`} value={formData.moderation_comment}
                onChange={(event) => {
                    setFormData({ ...formData, moderation_comment: event.target.value })
                }}
            />
