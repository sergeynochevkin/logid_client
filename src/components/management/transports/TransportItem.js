  ...
  ({
    ...
    useEffect(() => {
      if (Management.transport_images.find((el) => el.id === transport.id)) {
        setImages(
          Management.transport_images.find((el) => el.id === transport.id)
            .urlsArray
        );
      }
    ...
    const managementAction = async (option, field, new_value, id) => {
      try {
        await updateField(option, field, new_value, id).then((data) =>
          Notification.addNotification([
            {
              id: v4(),
              type: new_value === true ? "success" : "error",
              message: data,
            },
          ])
        );
    ...
    return (
      <div>
        <div className="management_row">
          ...
          {transport.side_type && (
            <div className="management_item">{transport.side_type}</div>
          )}
          {transport.load_capacity && (
            <div className="management_item">{transport.load_capacity}</div>
          )}
          {transport.ad_text && (
            <div className="management_item ad_text">{transport.ad_text}</div>
          )}
          <div className="management_item">
            {transport.ad_show ? "ad on" : "ad off"}
          </div>
          {transport.ad_show && (
            <div className="management_item">{transport.moderated}</div>
          )}
          {transport.moderation_comment === "" ||
          !transport.moderation_comment ? (
            <></>
          ) : (
            <div className="management_item">
          ...
          {transport.thermo_bag === true && (
            <div className="management_item">thermo bag</div>
          )}
          {transport.refrigerator_minus === true && (
            <div className="management_item">refrigerator minus</div>
          )}
          {transport.refrigerator_plus === true && (
            <div className="management_item">refrigerator_plus</div>
          )}
          {transport.hydraulic_platform === true && (
            <div className="management_item">hydraulic platform</div>
          )}
          {transport.side_loading === true && (
            <div className="management_item">side loading</div>
          )}
          {transport.glass_stand === true && (
            <div className="management_item">glass stand</div>
          )}
          <div className="management_images_container">
            {images.length > 0 ? (
              images.map((image) => (
                <img
                  src={image}
                  className="management_image_icon"
                  key={image}
                  onClick={() => {
