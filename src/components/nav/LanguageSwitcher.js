  ...
  return (
    <div className="language_switcher_anchor">
      <div className={`language_switcher_container ${Setting.app_theme}`}>
        <div
          className="nav_bar_item language_switch active"
          onClick={() => {
            if (!visible) {
              setVisible(true);
        ...
        >
          {languageList.find((el) => el.value === Translate.language).name}
        </div>

        {!user ||
        (user &&
          user.user.role !== "nmanager" &&
          user.user.role !== "admin") ? (
          <div ref={ref}>
            {visible && isComponentVisible ? (
              languageList
                .filter((el) => el.value !== Translate.language)
                .map((item) => (
                  <div
                    onClick={() => {
                    ...
                    className="nav_bar_item language_switch"
                    key={item.id}
                  >
                    {item.name}
                  </div>
