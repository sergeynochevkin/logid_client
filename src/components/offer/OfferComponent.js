      ...
      <>
        <VerticalContainer>
          ...
          {user.user.role === "carrier" ? (
            ...
            >
              {thisOrderOffers.find(
                (el) => el.carrierId === UserInfo.userInfo.id
              )
                ? SetNativeTranslate(Translate.language, {}, "change_offer")
                : SetNativeTranslate(Translate.language, {}, "make_offer")}
            </CardButton>
          ) : (
            <></>
          )}

          {thisOrderOffers.length > 0 ? (
            <CardRow
              onClick={(event) => {
                event.stopPropagation();
                setModalActive(true);
            ...
            >
              <CardColName>
                {SetNativeTranslate(Translate.language, {}, "total_offers")}
              </CardColName>
              <CardColName> {thisOrderOffers.length}</CardColName>
            </CardRow>
          ) : (
            <></>
          )}
