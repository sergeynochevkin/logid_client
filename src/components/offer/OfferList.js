...
const OfferList = ({
  ...
  return (
    <VerticalContainer style={{ alignItems: "center" }}>
      {thisOrderOffers.filter((el) => el.carrierId !== UserInfo.userInfo.id)
        .length > 0 ? (
        <Smaller>
      ...
      {thisOrderOffers.filter((el) => el.carrierId !== UserInfo.userInfo.id)
        ...
        >
          {user.user.role === "customer" || user.user.role === "driver" ? (
              ...
              <tbody>
                {thisOrderOffers.map((oneOffer) => (
                  <OfferItem
                    ...
                    user={user}
                    UserInfo={UserInfo}
                    noPartner={thisOrderNoPartners.find(
                      (el) => el.id === oneOffer.carrierId
                    )}
            ...
            <>
              {thisOrderOffers
                .filter((el) => el.carrierId !== UserInfo.userInfo.id)
                .sortOffers()
                .map((oneOffer) => (
                  <OfferItem
                    key={oneOffer.id}
