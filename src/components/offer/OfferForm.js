...
const OfferForm = observer(
  ({
    ...
    thisCarrierOffer
      ? (formData.this_carrier_offer_id = thisCarrierOffer.id)
      : (formData.this_carrier_offer_id = undefined);

    const validCost = /^\d+$/;

    formData.time_from = useInput(
      thisCarrierOffer
        ? setTime(new Date(thisCarrierOffer.time_from), 0, "form")
        : firstPoint
        ? setTime(new Date(firstPoint.time), 0, "form")
        : "",
      { isEmpty: true },
      SetNativeTranslate(
        Translate.language,
    ...
    formData.cost = useInput(
      thisCarrierOffer ? thisCarrierOffer.cost : "",
      { isEmpty: true, minLength: 2, maxLength: 6, validFormat: validCost },
      SetNativeTranslate(Translate.language, {}, "cost").toLowerCase()
    );
    formData.carrier_comment = useInput(
      thisCarrierOffer ? thisCarrierOffer.carrier_comment : "",
      { isEmpty: true, minLength: 8, maxLength: 70 },
      SetNativeTranslate(Translate.language, {}, "comment").toLowerCase()
    );

    useEffect(() => {
      formData.transportid = thisCarrierOffer
        ? thisCarrierOffer.transportid
        : Transport.transports.filter((el) => el.type === oneOrder.type)
            .length === 1
        ? Transport.transports.filter((el) => el.type === oneOrder.type).id
        : undefined;
    }, []);
    ...
    useEffect(() => {
      if (oneOrder.order_status === "new") {
        if (
          ...
          setFormData({
            ...formData,
            transportid: Transport.transports.find(
              (el) => el.type === oneOrder.type
            ).id,
          });
        }
      }
    ...
    const click = async (event) => {
      try {
        ...
        if (thisCarrierOffer) {
          ...
          Notification.addNotification([
            {
              ...
              message: SetNativeTranslate(Translate.language, {
                russian: [
                  "Вы изменили предложение к",
                  oneOrder.order_type === "order" ? `заказу` : `аукциону`,
                  oneOrder.id,
                ],
                english: [
                  "You have changed the offer to the",
                  oneOrder.order_type === "order" ? `order` : `auction`,
                  oneOrder.id,
                ],
                spanish: [
                  "Has cambiado la oferta a la",
                  oneOrder.order_type === "order" ? `orden` : `subasta`,
                  oneOrder.id,
                ],
                turkish: [
                  "Teklifi şu şekilde değiştirdiniz",
                  oneOrder.order_type === "order" ? `emir` : `açık arttırma`,
                  oneOrder.id,
                ],
              }),
            },
          ...
          await createOffer(Translate.language, formData);
          sendMail(
            Translate.language,
            user.user.role,
            oneOrder.id,
          ...
          Notification.addNotification([
            {
              ...
              message: SetNativeTranslate(Translate.language, {
                russian: [
                  "Вы сделали предложение к",
                  oneOrder.order_type === "order" ? `закау` : `аукциону`,
                  oneOrder.id,
                ],
                english: [
                  "You have made an offer to the",
                  oneOrder.order_type === "order" ? `закау` : `аукциону`,
                  oneOrder.id,
                ],
                spanish: [
                ...
                turkish: [
                  "Bir teklifte bulundun",
                  oneOrder.order_type === "order" ? `emir` : `açık arttırma`,
                  oneOrder.id,
                ],
                сhinese: [
                  "您已向",
                  oneOrder.order_type === "order" ? `扎考` : `拍卖`,
                  oneOrder.id,
                ],
                hindi: [
                  "आपने को एक प्रस्ताव दिया है",
                  oneOrder.order_type === "order" ? `ज़काउ` : `नीलामी`,
                  oneOrder.id,
                ],
              }),
            },
    ...
    const delOffer = async (event) => {
      try {
        event.preventDefault();
        await deleteOffer(thisCarrierOffer.id).then(
          sendMail(
            Translate.language,
            user.user.role,
            oneOrder.id,
        ...
        Notification.addNotification([
          {
            ...
            message: SetNativeTranslate(Translate.language, {
              russian: [
                "Вы удалили предложение к",
                oneOrder.order_type === "order" ? `закау` : `аукциону`,
                oneOrder.id,
              ],
              english: [
                "You have deleted an offer to the",
                oneOrder.order_type === "order" ? `закау` : `аукциону`,
                oneOrder.id,
              ],
              spanish: [
                "Has eliminado la oferta para",
                oneOrder.order_type === "order" ? `orden` : `subasta`,
                oneOrder.id,
              ],
              turkish: [
                "Şu teklifi kaldırdınız",
                oneOrder.order_type === "order" ? `emir` : `açık arttırma`,
                oneOrder.id,
              ],
              сhinese: [
      ...
      <Container>
        <Form>
          ...
          <VerticalContainer style={{ gap: "0px" }}>
            ...
            >
              {(formData.cost.isEmpty && formData.cost.isDirty) ||
              formData.cost.minLengthError ||
              formData.cost.maxLengthError ||
              formData.cost.formatError
          ...
          <VerticalContainer style={{ gap: "0px" }}>
            <Input
              ...
              value={formData.time_from.value}
              onChange={(e) => formData.time_from.onChange(e)}
              onBlur={(e) => formData.time_from.onBlur(e)}
              style={{
                borderLeft:
                  formData.time_from.notValid || formData.time_from.isEmpty
                    ? " solid 1px rgb(254, 111, 103,0.8)"
                    : "",
              }}
              name="time_from"
          ...
          <VerticalContainer style={{ gap: "0px" }}>
            <Input
              ...
              style={{
                borderLeft:
                  formData.carrier_comment.notValid &&
                  !formData.carrier_comment.isEmpty
                    ? " solid 1px rgb(254, 111, 103,0.8)"
                    : "",
              }}
              type="text"
              name="carrier_comment"
              id="carrier_comment"
            ></Input>
            <FieldName
              style={{
            ...
            >
              {formData.carrier_comment.minLengthError ||
              formData.carrier_comment.maxLengthError
              ...
              <div className="transport_selector_container">
                ...
                <div className="transport_items_container">
                  {Transport.transports
                    .filter((el) => el.type === oneOrder.type)
                    .map((item) => (
                      <TransportOfferSelectorItem
                        key={item.id}
                        thisTransport={item}
        ...
        <div className="offer_modal_buttons_container">
          ...
          {thisCarrierOffer ? (
            <CardButton onClick={delOffer}>
              {SetNativeTranslate(Translate.language, {}, "delete")}
            </CardButton>
          ) : (
            <></>
          )}
