...
const OfferItem = observer(
  ({
    ...
    useEffect(() => {
      if (
        Transport.transport_images.find((el) => el.id === oneOffer.transportid)
      ) {
        setImages(
          Transport.transport_images.find(
            (el) => el.id === oneOffer.transportid
          ).urlsArray
        );
        setImage(
          Transport.transport_images.find(
    ...
    const inWork = async () => {
      try {
        await updateOrder(
          ...
          user.user.role,
          "inWork",
          oneOrder.order_status,
          noPartner.id,
          UserInfo.userInfo.id,
          oneOffer.cost,
          oneOffer.time_from,
          firstPoint.id,
          oneOffer.transportid
        )
          .then(
            sendMail(
              Translate.language,
    ...
    const decline = async () => {
      try {
        await deleteOffer(oneOffer.id, user.user.role).then((data) =>
          Notification.addNotification([
            {
              id: v4(),
              type: "success",
    ...
    let thisPartnerInfo;
    noPartner &&
    Partner.partners.filter((el) => el.partnerUserInfoId === noPartner.id)
      .length > 0
      ? (thisPartner = Partner.partners.find(
          (el) => el.partnerUserInfoId === noPartner.id
        ))
      : (thisPartner = {});
    noPartner &&
    Partner.partnerInfos.filter((el) => el.id === noPartner.id).length > 0
      ? (thisPartnerInfo = Partner.partnerInfos.find(
          (el) => el.id === noPartner.id
        ))
      : (thisPartnerInfo = noPartner);
          ...
          <>
            <tr>
              ...
              <OrderTd
                style={{
                  backgroundColor:
                    thisPartner && thisPartner.status === "normal"
                      ? "rgb(241,196,15,0.8)"
                      : thisPartner && thisPartner.status === "priority"
                      ? "rgb(129, 199, 132,0.8)"
              ...
              <OrderTd>
                {Math.floor(thisPartnerInfo.total_rating * 100) / 100}
              </OrderTd>
              ...
              user.user.role === "driver" ? (
                ...
                <td>
                  <div className="offer_action_container">
                    <img
                      src={
                        Setting.app_theme === "dark" && showMoreInfo
                          ? arrow_up_dark_
                          : Setting.app_theme === "dark" && !showMoreInfo
                          ? info_dark
                          : Setting.app_theme === "light" && showMoreInfo
                          ? arrow_up_
                          : info
                      }
                    ...
                    />
                    <img
                      src={Setting.app_theme === "dark" ? done_dark : done}
                      className="offer_action_icon"
                      onClick={inWork}
                    />
                    <img
                      src={Setting.app_theme === "dark" ? close_dark : close}
                      className="offer_action_icon"
                      onClick={decline}
                    />
                  </div>
              ...
              {showMoreInfo === true ? (
                <>
                  <td colspan="8">
                    ...
                    >
                      {oneOffer.carrier_comment ? (
                        <CardColValue>{oneOffer.carrier_comment}</CardColValue>
                      ) : (
                        <></>
                      )}
                      <RatingView
