    ...
    <div>
      <VerticalContainer style={{ gap: "0px" }}>
        <Select
          ...
          style={{
            borderLeft: formData.driver_id.isEmpty
        ...
        >
          ...
          <option value={UserInfo.userInfo.id}>
            {SetNativeTranslate(Translate.language, {
              russian: ["Я"],
              english: ["I am"],
              spanish: ["I"],
          ...
          {Driver.drivers.map((driver) => (
            <option value={driver.user_info.id} key={driver.id}>
              {driver.user_info.name_surname_fathersname}
            </option>
          ))}
        </Select>
        <FieldName
