...
const OrderItem = observer(
  ({
    ...
    let thisDriverInfo;
    ComponentFunction.OrdersComponentFunction === "orderList"
      ? (thisPartner = onePartner)
      : ComponentFunction.OrdersComponentFunction === "orderItem"
      ? (thisPartner = Partner.partner)
      : (thisPartner = {});
    let thisPartnerInfo;
    ComponentFunction.OrdersComponentFunction === "orderList"
      ? (thisPartnerInfo = onePartnerInfo)
      : ComponentFunction.OrdersComponentFunction === "orderItem"
      ? (thisPartnerInfo = Partner.partnerInfo)
      : (thisPartnerInfo = {});
    ComponentFunction.OrdersComponentFunction === "orderList"
      ? (thisDriverInfo = driverInfo)
      : ComponentFunction.OrdersComponentFunction === "orderItem"
      ? (thisDriverInfo = Driver.driver)
      : (thisDriverInfo = {});

    let thisOrderOffers = oneOrderOffers;
    let thisCarrierOffer = thisOrderOffers.find(
      (el) => el.carrierId === UserInfo.userInfo.id
    );
    ...
    useEffect(() => {
      if (oneOrder.order_status === "inWork" && Transport.transports) {
        Transport.transport_by_order.find((el) => el.orderId === oneOrder.id) &&
          setTransport(
            Transport.transports.find(
              (el) =>
                el.id ===
                Transport.transport_by_order.find(
                  (el) => el.orderId === oneOrder.id
                ).transportId
            )
          );
      }
    ...
    useEffect(() => {
      if (
        ...
        oneOrder.order_status === "inWork"
      ) {
        if (Transport.transport_images.find((el) => el.id === transport.id)) {
          setImages(
            Transport.transport_images.find((el) => el.id === transport.id)
              .urlsArray
          );
        }
      }
    ...
    useEffect(() => {
      if (order.order_images.find((el) => el.id === oneOrder.id)) {
        setOrderImages(
          order.order_images.find((el) => el.id === oneOrder.id).urlsArray
        );
      }
    }, []);
    ...
    const toOrderItem = () => {
      ...
      Point.setThisOrderPoints(
        Point.divided_points[ComponentFunction.Function].filter(
          (el) => el.orderIntegrationId === order.order.pointsIntegrationId
        )
      );
      ...
      if (user.user.role === "carrier") {
        if (
          order.views[thisOrder.order_status] &&
          !order.views[thisOrder.order_status].find(
            (el) =>
              el.orderId === thisOrder.id &&
              el.userInfoId === UserInfo.userInfo.id
          )
        ) {
    ...
    return (
      <>
        {thisOrderPoints.length > 0 ? (
          ...
          >
            ...
            <VerticalContainer style={{ gap: "3px" }}>
              <div
                className="order_actions_container"
                onClick={(e) => {
                  e.stopPropagation();
                }}
              >
                ...
                thisOrder.order_status === "inWork" ? (
                  <ShareComponent parent="order_item" thisOrder={thisOrder} />
                ) : (
                  <></>
                )}
                ...
                <div className="nav_links_container">
                  {thisOrder.yandex_url && Adress.country.value === "russia" ? (
                    <div>
                      <a target="blank" href={thisOrder.yandex_url}>
                        <img
                          src={ya}
                  ...
                  {thisOrder.google_url && (
                    <div>
                      <a target="blank" href={thisOrder.google_url}>
                        <img
                          src={g}
                          alt={SetNativeTranslate(Translate.language, {
              ...
              <CardRow>
                  ...
                  <CardButton
                    onClick={(event) => {
                      ...
                      if (!order.group.includes(thisOrder.id)) {
                        ...
                        order.setGroup(
                          order.group.filter((el) => el !== thisOrder.id)
                        );
                      }
                    }}
                  >
                    {!order.group.includes(thisOrder.id)
                      ? SetNativeTranslate(Translate.language, {}, "select")
                      : SetNativeTranslate(Translate.language, {}, "deselect")}
                  </CardButton>
                ) : (
                  <></>
                )}
                ...
                <CardButton
                  onClick={(event) => {
                    ...
                    if (!State.user_state.favorite_order_state) {
                      ...
                      !State.user_state.favorite_order_state.includes(
                        thisOrder.id
                      )
                    ) {
                      State.setUserStateField(
                ...
                >
                  ...
                  (State.user_state.favorite_order_state &&
                    ...
                    ? SetNativeTranslate(Translate.language, {}, "to_favorites")
                    : State.user_state.favorite_order_state &&
                      State.user_state.favorite_order_state.includes(
                        thisOrder.id
                      )
              ...
              <CardRow>
                ...
                <CardColName>
                  {SetNativeTranslate(Translate.language, {}, "status")}
                </CardColName>
                <CardColValue>
                  {SetNativeTranslate(
                    Translate.language,
                      ...
                      <>
                        {onePartnerInfo.legal === "person" ? (
                          <CardColName
                            style={{
                              backgroundColor:
                                user.user.role !== "driver" && thisPartner
                                  ? setColor(thisPartner?.status)
                                  : "",
                            }}
                          >
              ...
              {thisDriverInfo &&
              onePartnerInfo &&
              thisDriverInfo.id !== onePartnerInfo.id ? (
                <CardRow
                  style={{ cursor: "pointer" }}
                  onClick={() => {
                ...
                >
                  ...
                  <CardColValue>
                    {thisDriverInfo.name_surname_fathersname}
                  </CardColValue>
                </CardRow>
              ) : (
                <></>
              ...
              thisPartnerInfo ? (
                <>
                  <CardRow>
                    <CardColName>
                      {SetNativeTranslate(Translate.language, {}, "phone")}
                    </CardColName>
              ...
              <CardRow>
                ...
                <CardColName>
                  {SetNativeTranslate(Translate.language, {}, "transport")}
                </CardColName>
                ...
                <CardColValue>
                  ...
                  transport ? (
                    ...
                    thisOrder.order_status === "inWork" &&
                    images.length > 0 ? (
                    <div className="in_order_transport_image_icon_container">
                      {images.length > 0 ? (
                        images.map((image) => (
                          <img
                            src={image}
                            className="in_order_transport_image_icon"
                            key={image}
                            onClick={(event) => {
                              event.stopPropagation();
                              setImage(image);
                        ...
                        <CardColValue>{thisOrder.type}</CardColValue>
                      )}
                    </div>
                  ) : (
              ...
              <CardRow>
                <CardColName>
                  {SetNativeTranslate(Translate.language, {}, "cost")}
                </CardColName>
                <CardColValue>
                  {thisOrder.cost === 0
                    ? SetNativeTranslate(
                    ...
                    : `${thisOrder.cost} ${Adress.country.currency}`}
                </CardColValue>
              </CardRow>

              {(ComponentFunction.Function === "new" ||
                ComponentFunction.Function === "postponed") &&
                user.user.role !== "driver" &&
                Partner.partnerInfos.length > 0 && (
                  <CardRow>
                    <CardColName>
                      {SetNativeTranslate(Translate.language, {}, "available")}
                    </CardColName>
                    {user.user.role === "customer" &&
                    Partner.partnerInfos.length > 0 &&
                    (thisOrder.order_status === "new" ||
                      thisOrder.order_status === "postponed") ? (
                      <CardColValue>
                        {!thisOrder.for_group && !thisOrder.for_partner
                          ? SetNativeTranslate(Translate.language, {}, "to_all")
                          : thisOrder.for_group
                          ? `${SetNativeTranslate(
                              Translate.language,
                              {},
                              ...
                              Partner.groups.find(
                                (el) => el.dataValues.id === thisOrder.for_group
                              ).dataValues.name
                            }`
                          : thisOrder.for_partner
                          ? `${SetNativeTranslate(
                              Translate.language,
                              {},
                              "to_partner"
                            )} ${
                              Partner.partnerInfos.find(
                                (el) => el.id === thisOrder.for_partner
                              ).legal !== "person"
                                ? Partner.partnerInfos.find(
                                    (el) => el.id === thisOrder.for_partner
                                  ).company_name
              ...
              <EquipmentRow>
                ...
                {thisOrder.refrigerator_minus === true ? (
                  <CardEquipment>
                    {SetNativeTranslate(
                      Translate.language,
                      {},
                ...
                {thisOrder.refrigerator_plus === true ? (
                  <CardEquipment>
                    {SetNativeTranslate(
                      Translate.language,
                      {},
                ...
                {thisOrder.hydraulic_platform === true ? (
                  <CardEquipment>
                    {SetNativeTranslate(
                      Translate.language,
                      {},
                ...
                {SetNativeTranslate(
                  Translate.language,
                  {
                    russian: [
                      "Просмотрен",
                      order.views[thisOrder.order_status] &&
                        order.views[thisOrder.order_status].filter(
                          (el) => el.orderId === thisOrder.id
                        ).length,
                      "раз",
                    ],
                    english: [
            ...
            <div className="in_order_transport_image_icon_container">
              {order_images.length > 0 ? (
                order_images.map((image) => (
                  <img
                    onClick={(event) => {
                      event.stopPropagation();
                      setImage(image);
        ...
        (ComponentFunction.Function === "new" ||
          ComponentFunction.Function === "postponed" ||
          ComponentFunction.Function === "inWork") ? (
          <MapComponent thisOrder={thisOrder} />
        ) : (
          <></>
        )}
        ...
        {thisDriverInfo && (
          <Modal modalActive={modalActive3} setModalActive={setModalActive3}>
            <DriverModalContent
              setModalActive={setModalActive3}
              onePartnerInfo={thisDriverInfo}
            />
          </Modal>
        )}
