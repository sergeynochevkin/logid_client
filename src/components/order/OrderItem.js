            ...
            >
                {ComponentFunction.OrdersComponentFunction === 'orderItem' ?
                    <AddDeleteFieldButton onClick={(event) => {
                        ...
                        setFetchStart(true)
                    }}>{SetTranslate(Translate.language,'back_to_order_list')}</AddDeleteFieldButton>
                    : <></>}
                ...
                <VerticalContainer style={{ gap: '3px' }}>
                    ...
                    <CardRow>
                        {(user.user.role === 'carrier' && thisOrder.order_status === 'new') || (thisOrder.order_status === 'inWork') ?
                            ...
                            : ComponentFunction.OrdersComponentFunction !== 'orderItem' ?
                                ...
                                >{!order.group.includes(thisOrder.id) ? SetTranslate(Translate.language,'select') : SetTranslate(Translate.language,'deselect')}</CardButton>
                                : <></>
                        }
                        ...
                        >{State.user_state.favorite_order_state && !State.user_state.favorite_order_state.includes(thisOrder.id) ? SetTranslate(Translate.language,'to_favorites') : SetTranslate(Translate.language,'from_favorites')}
                        </CardButton>
                    ...
                    <CardRow>
                        <CardColName
                        >{
                                thisOrder.order_type === "order" ? SetTranslate(Translate.language,'order') :
                                    thisOrder.order_type === "auction" ? SetTranslate(Translate.language,'auction') :
                                        <></>
                            }</CardColName>
                        <CardColValue>{thisOrder.id}</CardColValue>
                        <CardColName>{SetTranslate(Translate.language,'status')}</CardColName>
                        <CardColValue>{SetTranslate(Translate.language,thisOrder.order_status)}</CardColValue>
                    </CardRow>
                    ...
                    {onePartnerInfo ?
                        <>
                            <CardRow
                                ...
                                <CardColName>
                                    {user.user.role === 'carrier' ? SetTranslate(Translate.language,'customer') :
                                        user.user.role === 'customer' ? SetTranslate(Translate.language,'carrier') : ''}
                                </CardColName>
                    ...
                    {ComponentFunction.OrdersComponentFunction === 'orderItem' && (thisOrder.order_status !== 'new' && thisOrder.order_status !== 'postponed' && thisOrder.order_status !== 'canceled') && thisPartnerInfo ?
                        <>
                            <CardRow>
                                <CardColName>{SetTranslate(Translate.language,'phone')}</CardColName>
                    ...
                    {thisOrder.order_comment !== '' ? <CardRow>
                        <CardColName>{SetTranslate(Translate.language,'order_comment')}</CardColName>
                        <CardColValue>{thisOrder.order_comment}</CardColValue>
                    </CardRow> : <></>}

                    <CardRow>
                        <CardColName>{SetTranslate(Translate.language,'transport')}</CardColName>
                        <CardColValue>{SetTranslate(Translate.language,thisOrder.type)}</CardColValue>
                    </CardRow>
                    {(thisOrder.side_type || thisOrder.load_capacity) &&
                        <CardRow>
                            {thisOrder.side_type && <CardColValue> {SetTranslate(Translate.language,thisOrder.side_type)}</CardColValue>}
                            {thisOrder.load_capacity && <CardColValue> {SetTranslate(Translate.language,thisOrder.load_capacity)}</CardColValue>}
                        </CardRow>
                    }
                    <CardRow>
                        <CardColName>{SetTranslate(Translate.language,'cost')}</CardColName>
                        <CardColValue>{thisOrder.cost === 0 ? SetTranslate(Translate.language,'not_specified') : thisOrder.cost}</CardColValue>
                    </CardRow>

                    {(ComponentFunction.Function === 'new' || ComponentFunction.Function === 'postponed') &&
                        <CardRow>
                            <CardColName>{SetTranslate(Translate.language,'available')}</CardColName>
                            {user.user.role === 'customer' && (thisOrder.order_status === 'new' || thisOrder.order_status === 'postponed') ?
                                <CardColValue>
                                    {for_group.length === 0 && for_partner.length === 0 ? SetTranslate(Translate.language,'to_all') : for_group.length !== 0 ? `${SetTranslate(Translate.language,'to_group')} ${groups}` : for_partner.length !== 0 ? `${SetTranslate(Translate.language,'to_partner')} ${partnerNames}` : ''}
                                </CardColValue> :
                                user.user.role === 'carrier' && thisOrder.order_status === 'new' ?
                                    <CardColValue>
                                        {for_group.length === 0 && for_partner.length === 0 ? SetTranslate(Translate.language,'to_all') : for_group.length !== 0 ? SetTranslate(Translate.language,'your_group') : for_partner.length !== 0 ? SetTranslate(Translate.language,'to_you') : ''}
                                    </CardColValue> : <></>}
                        </CardRow>
                    }

                    <EquipmentRow>
                        {thisOrder.thermo_bag === true ? <CardEquipment>{SetTranslate(Translate.language,'thermo_bag')}</CardEquipment> : <></>}
                        {thisOrder.thermo_van === true ? <CardEquipment>{SetTranslate(Translate.language,'thermo_van')}</CardEquipment> : <></>}
                        {thisOrder.refrigerator_minus === true ? <CardEquipment>{SetTranslate(Translate.language,'refrigerator_minus')}</CardEquipment> : <></>}
                        {thisOrder.refrigerator_plus === true ? <CardEquipment>{SetTranslate(Translate.language,'refrigerator_plus')}</CardEquipment> : <></>}
                        {thisOrder.hydraulic_platform === true ? <CardEquipment>{SetTranslate(Translate.language,'hydraulic_platform')}</CardEquipment> : <></>}
                        {thisOrder.side_loading === true ? <CardEquipment>{SetTranslate(Translate.language,'side_loading')}</CardEquipment> : <></>}
                        {thisOrder.glass_stand === true ? <CardEquipment>{SetTranslate(Translate.language,'glass_stand')}</CardEquipment> : <></>}
                    </EquipmentRow>
                </VerticalContainer>
                <VerticalContainer>
                    <OrderStatusButtons
