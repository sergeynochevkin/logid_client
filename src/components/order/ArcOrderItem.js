...
const ArcOrderItem = observer(({ thisPoints, oneArcOrder }) => {
  ...
  const form_from_template = SetNativeTranslate(
    Translate.language,
    {},
    "form_from_template"
  );
  const check_restored_arc = SetNativeTranslate(
    Translate.language,
    {},
    "check_restored_arc"
  );
    ...
    Notification.addNotification([
      {
        id: v4(),
        type: "error",
        message: `${you_deleted} ${the.toLowerCase()} ${
    ...
    <>
      {thisPoints.length > 0 ? (
        <tr className="arc_table_row">
          ...
          <OrderTd>
            {SetNativeTranslate(Translate.language, {}, oneArcOrder.order_type)}
          </OrderTd>
          {firstPoint ? (
            <>
          ...
          <OrderTd>
            {SetNativeTranslate(Translate.language, {}, oneArcOrder.type)}
          </OrderTd>
          <OrderTd>
            {oneArcOrder.cost === 0
              ? SetNativeTranslate(Translate.language, {}, "not_specified")
              : oneArcOrder.cost}
          </OrderTd>
          {ComponentFunction.Function === "arc" ? (
            <OrderTd>
              {SetNativeTranslate(
                Translate.language,
                {},
                oneArcOrder.order_final_status
              )}
            </OrderTd>
          ) : (
            <></>
                    ...
                    Notification.addNotification([
                      {
                        ...
                        message: `${you_opened} ${
                          ComponentFunction.Function === "arc"
                            ? form_from_order
                            : form_from_template
                        } ${oneArcOrder.id}, ${check_restored_arc}`,
                      },
                    ]);
                  }}
                />
          ...
          <td>
            <div className="order_list_icon_container">
              ...
              (ComponentFunction.Function === "arc" &&
                oneArcOrder.order_final_status === "canceled") ? (
                <>
                  <img
                    className={"order_action_icon"}
