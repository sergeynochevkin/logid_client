  ...
  ({
    ...
    useEffect(() => {
      if (
        ...
        let lng;
        if (
          parseFloat(order.order.start_lat) > parseFloat(order.order.end_lat)
        ) {
          lat =
        ...
        if (
          parseFloat(order.order.start_lng) > parseFloat(order.order.end_lng)
        ) {
          lng =
    ...
    const go_to_order = SetNativeTranslate(
      Translate.language,
      {},
      "go_to_order"
    );
        ...
        if (thisOrder && thisOrder.order_status === "inWork") {
          ...
          if (gMap && driverInfo && driverInfo.user_info.location) {
            ...
            let title = SetNativeTranslate(Translate.language, {
              ...
              сhinese: [
                `更新 ${setTime(new Date(location.updated), 0, "show")}`,
              ],
              hindi: [
                `अद्यतन ${setTime(new Date(location.updated), 0, "show")}`,
              ],
            });
            if (!locationMarker) {
              setMarker(title, gMap, location, icon);
    ...
    useEffect(() => {
      if (
        ...
        if (gMap && UserInfo.userInfo.location) {
          ...
          let icon = UserInfo.images.find(
            (el) => el.id === UserInfo.userInfo.id
          ).urlsArray[1];
          let title = SetNativeTranslate(Translate.language, {
            russian: [
              `Обновлено ${setTime(new Date(location.updated), 0, "show")}`,
    ...
    useEffect(() => {
      if (
        ...
        if (
          ...
          for (const orderItem of order.map_orders) {
            let points = Point.divided_points[
