...
const User = observer(() => {
  ...
  useEffect(() => {
    if (
      ...
      (!Transport.transports || Transport.transports.length === 0)
    ) {
  ...
  useEffect(() => {
    if (link.order.id) {
      let i = 0;
      let delay =
        link.internet_speed < 5 ? 40 : link.internet_speed < 20 ? 20 : 10;
      ...
      let interval = setInterval(() => {
        if (order.totalCount[ComponentFunction.Function] > 0) {
          ...
          if (
            !order.divided_orders[ComponentFunction.Function].find(
              (el) => el.id === parseInt(link.order.id)
            )
          ) {
  ...
  return (
    <div className={`user_page_container ${Setting.app_theme}`}>
          ...
          <div className="scroll_content_container">
            <BookMark
              ...
              style={{
                ...
                cursor:
                  ComponentFunction.PageFunction === "orderList" && "default",
              }}
            >
              {SetNativeTranslate(Translate.language, {}, "orders")}
            </BookMark>
            ...
            {user.user.role === "customer" && (
              <BookMark
                ...
                style={{
                  color:
                    ComponentFunction.PageFunction === "orderForm" && "grey",
                  cursor:
                    ComponentFunction.PageFunction === "orderForm" && "default",
                }}
              >
                {ComponentFunction.orderFormFunction === "edit"
                  ? SetNativeTranslate(Translate.language, {}, "order_editing")
                  : SetNativeTranslate(Translate.language, {}, "create_order")}
              </BookMark>
            )}
            ...
            {user.user.role === "carrier" || user.user.role === "driver" ? (
              <>
                <BookMark
                  onClick={() => {
                    ComponentFunction.PageFunction !== "transport" &&
                      setFunction(false, false, "transport", false);
                  }}
                  style={{
                    color:
                      ComponentFunction.PageFunction === "transport" && "grey",
                    cursor:
                      ComponentFunction.PageFunction === "transport" &&
                      "default",
                  }}
                >
                  {SetNativeTranslate(Translate.language, {}, "transports")}
                </BookMark>
              </>
            ) : (
              <></>
              ...
              <BookMark
                onClick={() => {
                  ComponentFunction.PageFunction !== "drivers" &&
                    setFunction(false, false, "drivers", false);
                }}
                style={{
                  color: ComponentFunction.PageFunction === "drivers" && "grey",
                  cursor:
                    ComponentFunction.PageFunction === "drivers" && "default",
                }}
              >
            ...
            {user.user.role !== "driver" && (
              <BookMark
                onClick={() => {
                  ComponentFunction.PageFunction !== "partners" &&
                    setFunction("partners", false, "partners", false);
                }}
                style={{
                  color:
                    ComponentFunction.PageFunction === "partners" && "grey",
                  cursor:
                    ComponentFunction.PageFunction === "partners" && "default",
                }}
              >
                {SetNativeTranslate(
                  Translate.language,
                  {},
                  user.user.role === "carrier" ? "customers" : "carriers"
                )}
              </BookMark>
            )}
            ...
            <BookMark
              onClick={() => {
                ComponentFunction.PageFunction !== "account" &&
                  setFunction(false, false, "account", false);
              }}
              style={{
                color: ComponentFunction.PageFunction === "account" && "grey",
                cursor:
                  ComponentFunction.PageFunction === "account" && "default",
              }}
            >
              {SetNativeTranslate(Translate.language, {}, "account")}
            </BookMark>
      ...
      {ComponentFunction.PageFunction === "orderList" ? (
        ...
        <Partners />
      ) : ComponentFunction.PageFunction === "settings" ? (
        <SettingsComponent />
      ) : ComponentFunction.PageFunction === "drivers" ? (
        <DriverComponent />
      ) : (
