...
const BoardListItem = observer(({ transport }) => {
    ...
    useEffect(() => {
        if (Ad.users[Ad.transport_option].find(el => el.transport_id === transport.id)) {
            setAdUser(Ad.users[Ad.transport_option].find(el => el.transport_id === transport.id))
        }
        if (Ad.transport_images[Ad.transport_option].find(el => el.id === transport.id)) {
            // setImages(Ad.transport_images.find(el => el.id === transport.id).urlsArray)
            setMainImage(Ad.transport_images[Ad.transport_option].find(el => el.id === transport.id).urlsArray[0])
        }
