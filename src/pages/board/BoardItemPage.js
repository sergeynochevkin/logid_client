...
const BoardItemPage = observer(() => {
  ...
  useEffect(() => {
    setTransport({
      ...Ad.transports[Ad.transport_option].find((el) => el.id === id),
    });
  ...
  useEffect(() => {
    if (Ad.users[Ad.transport_option].find((el) => el.transport_id === id)) {
      setAdUser(
        Ad.users[Ad.transport_option].find((el) => el.transport_id === id)
      );
    }
    if (
      ...
      setImages(
        Ad.transport_images[Ad.transport_option].find((el) => el.id === id)
          .urlsArray
      );
      setMainImage(
        Ad.transport_images[Ad.transport_option].find((el) => el.id === id)
    ...
    <>
      <Modal modalActive={modalActive} setModalActive={setModalActive}>
        <div className="board_item_page_image_modal_container">
          <img src={mainImage} className="board_item_page_modal_image"></img>
        </div>
      </Modal>
      <Modal setModalActive={setModalActive1} modalActive={modalActive1}>
        <Auth
          ...
          after_action={{
            action: "transport_contact_viewed",
          ...
          <div className="board_item_ad_container">
            <div className="board_item_page_header_container">
              <div className="board_item_page_header_back_container">
                ...
                <div className="board_item_page_header">{`${SetNativeTranslate(
                  Translate.language,
                  {},
                  transport.type
                )} ${
            ...
            <div className="board_item_page_images_container">
              <div className="board_item_page_main_image_container">
                <img
                  className="board_item_page_main_image"
                  src={mainImage}
                  onClick={() => {
                    setModalActive(true);
                  }}
              ...
              <div className="board_item_page_image_icons_container">
                {images.length > 0 ? (
                  images.map((image) => (
                    <img
                      style={{
                        filter: image === mainImage ? "brightness(0.5)" : "",
                      }}
                      onClick={() => {
                        setMainImage(image);
                      }}
                      src={image}
                      className="board_item_page_image_icon"
                      key={image}
                    ></img>
                  ))
                ) : (
            ...
            <div className="board_item_page_equipment_container">
              <div className="board_item_page_equipment_row">
                {transport.thermo_bag === true ? (
                  <div className="board_item_page_equipment_item">
            ...
            <div className="board_item_page_info_container">
              <div className="board_item_page_info">
                ...
                <div>
                  {(user.isAuth && showContact) ||
                  transport.userInfoId === UserInfo.userInfo.id ? (
                    ad_user.phone
                  ) : (
                    <Button
                      onClick={() => {
            ...
            <div className="board_item_page_info_statistics_container">
              ...
              <div className="board_item_page_info_statistics_rating">
                {ad_user.rating === "no_rating"
                  ? SetNativeTranslate(Translate.language, {
                      russian: ["нет оценок"],
                      english: ["no ratings"],
              ...
              <img
                className="board_item_page_icon"
                src={Setting.app_theme === "light" ? eye : eye_dark}
              ></img>
              <div>{ad_user.viewed}</div>
              <div>({ad_user.viewed_today})</div>
              <img
                className="board_item_page_icon"
                src={Setting.app_theme === "light" ? phone : phone_dark}
              ></img>
