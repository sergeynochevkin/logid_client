...
const AdTransportItem = observer(({ transport, transports }) => {
    ...
    return (<>
        ...
        <div className={`ad_transport_item ${Setting.app_theme}`} >
            ...
            <div className='ad_transport_item_ad_header_container'>{`${SetNativeTranslate(Translate.language, {}, transport.type)} ${transport.type === 'minibus' || transport.type === 'truck' ? SetNativeTranslate(Translate.language, {}, transport.load_capacity) : ''}`}</div>
            
            <div className='ad_transport_item_ad_text_container'>{transport.ad_text}</div>
