...
const MainSection = observer(
  ({ section, items, callRequested, setCallRequested }) => {
    const { Setting } = useContext(SettingContext);
    const { user } = useContext(UserContext);

    return (
      <div
        className={`section_container ${section.class} ${
          Setting.app_theme === "light" ? "" : "dark"
        }`}
      >
          ...
          <div>
            <div className="section_content_container">
              ...
              >
                <div className="main_section_items_container">
                  {items
                    .filter((el) => el.id >= 1 && el.id <= 16)
                    .map((item) => (
                      <MainSectionItem item={item} key={item.id} />
                    ))}
                </div>
              </div>
            </div>
            <div className="swipe_icon_container">
              <img
                ...
                src={Setting.app_theme === "light" ? swipe : swipe_dark}
              />
            </div>
          </div>
        ...
        {!user.user.role && !callRequested ? (
          <CaptureForm setCallRequested={setCallRequested} section={section} />
        ) : (
          <></>
        )}
      </div>
