...
import { setOrderViewed } from "./http/orderApi";
import { fetchUser } from "./http/userAPI";
import {
  fetchManagementClicks,
  fetchManagementOrders,
  fetchManagementRegistrations,
  fetchManagementTransports,
  fetchManagementUsers,
  fetchManagementVisits,
} from "./http/managementApi";
...
const Fetcher = observer(() => {
    ...
    for (const transport of transports) {
      if (!Transport.transports.find((el) => el.id === transport.id)) {
        ...
        if (fileNames) {
          ...
          Transport.setTransportImages([
            ...Transport.transport_images,
            transportImageObject,
          ]);
        }
      }
    }
  ...
  let orderImageHandler = async (orders) => {
    for (const oneOrder of orders) {
      if (
        !order.order_images.find((el) => el.id === oneOrder.id) ||
        JSON.stringify(
          order.order_images.find((el) => el.id === oneOrder.id).urlsArray
        ) !== JSON.stringify(oneOrder.files)
      ) {
        ...
        if (fileNames) {
          ...
          let data = [
            ...order.order_images.filter((el) => el.id !== oneOrder.id),
            orderImageObject,
          ];
  ...
  let imageHandler = async (objects) => {
    for (const object of objects) {
      if (
        !UserInfo.images.find((el) => el.id === object.id) ||
        JSON.stringify(
          UserInfo.images.find((el) => el.id === object.id).urlsArray
        ) !== JSON.stringify(object.files)
      ) {
        let imageObject = {
          id: object.id,
          urlsArray: [],
        ...
        if (object.files) {
          ...
          let data = [
            ...UserInfo.images.filter((el) => el.id !== object.id),
            imageObject,
          ];
  ...
  let partnerImageHandler = async (objects) => {
    for (const object of objects) {
      if (
        !Partner.images.find((el) => el.id === object.id) ||
        JSON.stringify(
          Partner.images.find((el) => el.id === object.id).urlsArray
        ) !== JSON.stringify(object.files)
      ) {
        let imageObject = {
          id: object.id,
        ...
        if (object.files) {
          ...
          let data = [
            ...Partner.images.filter((el) => el.id !== object.id),
            imageObject,
          ];
  ...
  let managementUserImageHandler = async (objects) => {
    for (const object of objects) {
      if (
        !Management.user_images.find((el) => el.id === object.id) ||
        JSON.stringify(
          Management.images.find((el) => el.id === object.id).urlsArray
        ) !== JSON.stringify(object.files)
      ) {
        let imageObject = {
          id: object.id,
        ...
        if (object.files) {
          ...
          let data = [
            ...Management.user_images.filter((el) => el.id !== object.id),
            imageObject,
          ];
  ...
  let driverImageHandler = async (objects) => {
    for (const object of objects) {
      if (
        !Driver.images.find((el) => el.id === object.user_info.id) ||
        JSON.stringify(
          Driver.images.find((el) => el.id === object.user_info.id).urlsArray
        ) !== JSON.stringify(object.user_info.files)
      ) {
        let imageObject = {
        ...
        if (object.user_info.files) {
          ...
          let data = [
            ...Driver.images.filter((el) => el.id !== object.id),
            imageObject,
          ];
  ...
  let adImageHandler = async (transports, option) => {
    for (const transport of transports) {
      if (!Ad.transport_images[option].find((el) => el.id === transport.id)) {
        ...
        Ad.setTransportImages(
          [...Ad.transport_images[option], transportImageObject],
          option
        );
      }
    }
    ...
    if (UserInfo && Object.keys(UserInfo.userInfo).length > 0) {
      async function fetch() {
        await fetchNotifications(UserInfo.userInfo.id).then(async (data) => {
          Notification.setServerNotifications(
            data.filter((el) => el.viewed === true)
          );
          Notification.setNewServerNotifications(
            data.filter((el) => el.viewed === false)
          );
        });
      }
  ...
  useEffect(() => {
    Notification.new_server_notifications.forEach(async (element) => {
      Notification.addNotification([
        { id: element.uuid, type: element.type, message: element.message },
      ]);
    });
    ...
    if (UserInfo && Object.keys(UserInfo.userInfo).length > 0) {
      async function fetch() {
        if (Object.keys(UserInfo.userInfo).length !== 0) {
          await fetchUserLimits(UserInfo.userInfo.id).then((data) =>
            Limit.setUserLimits(data)
          );
          await fetchSubscription(UserInfo.userInfo.id).then((data) =>
        ...
        if (Object.keys(UserInfo.userInfo).length !== 0) {
          await fetchUserState(UserInfo.userInfo.id).then((data) => {
            ...
            user.user.role === "driver" &&
              State.setSupervisorState(data.supervisor_state);
          });
        }
      }
      fetch();
        ...
        if (Object.keys(UserInfo.userInfo).length !== 0) {
          await fetchPartners(UserInfo.userInfo.id).then(async (data) => {
            await fetchGroups(
              ...
              data.map((el) => el.partnerUserInfoId)
            ).then((data) => Partner.setGroups(data));
            await fetchOtherRatings(UserInfo.userInfo.id).then((data) => {
              Rating.setOtherRatings(data);
            });
            Partner.setPartner(
              data.find((el) => el.partnerUserInfoId === order.order.carrierId)
            );
            Partner.setMyBlocked(
              data
                .filter((el) => el.status === "blocked")
                .map((el) => el.partnerUserInfoId)
            );
            Partner.setMyFavorite(
              data
                .filter((el) => el.status === "favorite")
                .map((el) => el.partnerUserInfoId)
            );
            Partner.setPartners(data);
            await fetchUserInfos(
              data.map((el) => el.partnerUserInfoId),
              FilterAndSort.partnerFilters
            ).then((data) => {
      ...
      async function fetch() {
        if (Object.keys(UserInfo.userInfo).length !== 0) {
          await fetchPartners(UserInfo.userInfo.id).then(async (data) => {
            ...
            Partner.setPartner(
              data.find((el) => el.partnerUserInfoId === order.order.userInfoId)
            );
            await fetchUserInfos(
              data.map((el) => el.partnerUserInfoId),
              FilterAndSort.partnerFilters
            ...
            if (user.user.role === "carrier" || user.user.role === "driver") {
              ...
              user.user.role !== "driver" &&
                Partner.setMyFavorite(
                  data
                    .filter((el) => el.status === "priority")
                    .map((el) => el.partnerUserInfoId)
                );
              await fetchPartners(UserInfo.userInfo.id).then(async (data) => {
                Partner.setPartners(data);
                user.user.role !== "driver" &&
                  Partner.setIAmBlocked(
                    data
                      .filter((el) => el.status === "blocked")
                      .map((el) => el.userInfoId)
                  );
                user.user.role !== "driver" &&
                  Partner.setIAmFavorite(
                    data
                      .filter((el) => el.status === "favorite")
                      .map((el) => el.userInfoId)
                  );
  ...
  useEffect(() => {
    async function fetch() {
      await fetchAdTransports(
        FilterAndSort.boardFilters,
        Ad.transport_option,
        user.isAuth ? UserInfo.userInfo.id : ""
      ).then((data) => {
  ...
  useEffect(() => {
    async function fetch() {
      await fetchManagementUsers(
        user.user.id,
        FilterAndSort.managementFilters
      ).then((data) => {
  ...
  useEffect(() => {
    async function fetch() {
      await fetchSettings(UserInfo.userInfo.id).then((data) =>
        Setting.setUserSettings(data)
      );
    }
  ...
  return (
    <>
      ...
      <ReactInternetSpeedMeter
        ...
        threshold={1000}
        imageUrl={
          "https://thumbs.dreamstime.com/b/%D0%B8%D0%B7%D0%BE%D0%BB%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%BD%D0%B0%D1%8F-%D0%BA%D0%B2%D0%B0%D1%80%D1%82%D0%B8%D1%80%D0%B0-%D0%B7%D0%BD%D0%B0%D1%87%D0%BA%D0%B0-apple-%D0%B2%D0%B5%D0%BA%D1%82%D0%BE%D1%80%D0%BD%D0%B0%D1%8F-%D0%BA%D0%BE%D0%BD%D1%81%D1%82%D1%80%D1%83%D0%BA%D1%86%D0%B8%D1%8F-%D1%8F%D0%B1%D0%BB%D0%BE%D1%87%D0%BD%D1%8B%D1%85-165328587.jpg"
        }
