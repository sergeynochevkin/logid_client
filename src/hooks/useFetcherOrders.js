  ...
  async function fetch(order_status) {
    if (
      ...
      await fetchOrders(
        ...
        data.total_count &&
          Transport.setTransportByOrder(data.total_count.transport);

        order.setMapOrders(data.map_rows);
        if (data.rows.length !== 0) {
          await fetchPoints(
            data.rows.map((el) => el.pointsIntegrationId),
            UserInfo.userInfo.id
          ).then((data) => {
        ...
        if (
          ComponentFunction.OrdersComponentFunction === "orderItem" &&
          data.rows.find((el) => el.id === order.order.id)
        ) {
        ...
        if (order_status === "completed" && data.length !== 0) {
          await fetchOrderRatings(
            ...
            UserInfo.userInfo.id
          ).then((data) => Rating.setOrderRatings(data));
        }
        if (
          (order_status === "new" || order_status === "postponed") &&
          data.length !== 0
        ) {
          await fetchOffers(
            data.rows
              .filter((el) => el.order_type !== "order")
              .map((el) => el.id),
            UserInfo.userInfo.id
          ).then(async (data) => {
            ...
            await fetchUserInfos(
              Offer.offers.map((el) => el.carrierId),
              FilterAndSort.partnerFilters
            ).then((data) => Partner.setNoPartnerInfos(data));
          });
        }
      });
    }
