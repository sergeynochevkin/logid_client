    ...
    for (const validation in validations) {
      switch (validation) {
        case "isEmpty":
          value
            ? setIsEmpty(false)
            : setIsEmpty(true) || setErrorMessage(`${fieldName} ${not_empty}`);
          break;
        case "minLength":
          value.length < validations[validation] && value.length > 0
            ? setMinLengthError(true) ||
              setErrorMessage(
                `${fieldName} ${to_short} ${validations.minLength} ${symbols}`
              )
        ...
        case "maxLength":
          value.length > validations[validation] && value.length > 0
            ? setMaxLengthError(true) ||
              setErrorMessage(
                `${fieldName} ${to_long} ${validations.maxLength} ${symbols}`
              )
            : setMaxLengthError(false);
          break;
        case "validFormat":
          !validations[validation].test(String(value)) &&
          value.length &&
          value.length <= validations.maxLength &&
          value.length >= validations.minLength
            ? setFormatError(true) ||
              setErrorMessage(`${fieldName} ${format_error}`)
